...
def build_full_fix_prompt(
    *,
    ...
    """Build strict JSON contract prompt for chart-level full-fix generation."""
    allowed_list = "\n".join(f"- {path}" for path in allowed_files) or "- (none)"
    ...
    contract = (
        ...
        '  "result": "ok | no_change | error",\n'
        '  "summary": "short explanation",\n'
        '  "values_patch": {"key": "value"},\n'
        '  "template_patches": [\n'
        "    {\n"
        '      "file": "templates/deployment.yaml",\n'
        '      "purpose": "why this patch",\n'
        '      "unified_diff": "--- a/templates/deployment.yaml\\n+++ b/templates/deployment.yaml\\n@@ ...",\n'
        '      "updated_content": "full updated file content (preferred for reliability)"\n'
        "    }\n"
        "  ],\n"
        '  "violation_coverage": [\n'
        "    {\n"
        '      "rule_id": "PRB001",\n'
        '      "status": "addressed | unchanged | not_applicable | error",\n'
    ...
    prompt_parts = [
        "You are generating a complete Helm chart fix bundle.",
        "Return output as a single JSON object only.",
        "Do not use markdown, code fences, prose, or extra keys.",
        "",
        "Task:",
        task.strip(),
        "",
        "Allowed template files (strict allowlist):",
        allowed_list,
        "",
        "JSON response contract:",
        contract,
        "",
        "Rules:",
        "- `values_patch` must only include values keys to merge into values.yaml.",
        "- Put concrete desired configuration values in `values_patch` (values.yaml), not as hardcoded literals in template manifests.",
        "- Template patches must wire configuration from `.Values` references instead of embedding concrete final values.",
        "- Use canonical Kubernetes/value key names and hierarchy; keep casing consistent with current chart templates.",
        "- Do not invent alias or suffixed keys (for example `resourcesAutomation`, `replicaCountAutomation`).",
        "- Prefer canonical keys such as `replicaCount`, `resources.requests/limits`, and probe keys (`livenessProbe`, `readinessProbe`, `startupProbe`) when relevant.",
        "- Every `template_patches[].file` must be one of the allowed template files.",
        "- Always provide `template_patches[].updated_content` (full updated file content).",
        "- `unified_diff` is optional and may be empty.",
        "- If `unified_diff` is provided, it must be valid unified diff for that file.",
        "- Each template patch must include at least one of `updated_content` or `unified_diff`.",
        "- If no change is needed: set `result` to `no_change` and keep patch collections empty.",
        "- If unable to produce safe patch: set `result` to `error` and explain in `error`.",
    ]
    if context_lines:
        prompt_parts.extend(["", *context_lines])
...
def parse_full_fix_response(raw_text: str) -> FullFixResponse:
    """Parse strict JSON response (or fenced JSON) into full-fix model."""
    payloads = _parse_json_payloads(raw_text)
    last_error: ValidationError | None = None
    for payload in payloads:
        try:
            return FullFixResponse.model_validate(payload)
        except ValidationError as exc:
            last_error = exc
            continue
    if last_error is not None:
...
def format_full_fix_preview_markdown(
    response: FullFixResponse,
) -> str:
    """Render parsed full-fix response for dialog preview."""
    lines = [
        "### AI Full Fix Result",
        f"- **Result:** `{response.result.upper()}`",
        f"- **Summary:** {response.summary}",
        f"- **Values Patch Keys:** `{len(response.values_patch)}`",
        f"- **Template Patches:** `{len(response.template_patches)}`",
    ]
    if response.warnings:
        lines.extend(
            [
    ...
    if response.violation_coverage:
        lines.append("")
        lines.append("### Violation Coverage")
        for item in response.violation_coverage:
            lines.append(
                f"- `{item.rule_id}`: `{item.status.upper()}`"
                + (f" - {item.note}" if item.note else "")
            )
    if response.values_patch:
        lines.extend(
            [
                "",
                "### Values Patch",
                "```yaml",
                json.dumps(response.values_patch, indent=2),
                "```",
            ]
        )
    if response.template_patches:
        lines.append("")
        lines.append("### Template Patch Preview")
        for patch in response.template_patches:
            preview_body = patch.unified_diff.rstrip()
            preview_lang = "diff"
            if not preview_body and patch.updated_content.strip():
                preview_body = patch.updated_content.rstrip()
                preview_lang = "yaml"
            lines.extend(
                [
                    f"- **File:** `{patch.file}`",
