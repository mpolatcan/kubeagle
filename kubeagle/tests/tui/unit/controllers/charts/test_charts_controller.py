...
class TestChartsController:
    ...
    def test_controller_initializes_team_mapper_from_codeowners(
        self, tmp_path: Path
    ) -> None:
        """Test ChartsController wires TeamMapper when codeowners path is provided."""
        codeowners = tmp_path / "CODEOWNERS"
        codeowners.write_text("/my-chart/ @org/platform\n")

        controller = ChartsController(repo_path=tmp_path, codeowners_path=codeowners)
        assert controller.team_mapper is not None
        assert controller._extract_team({}, chart_name="my-chart") == "Platform"

    def test_controller_auto_detects_codeowners_in_repo(self, tmp_path: Path) -> None:
        """Test ChartsController auto-detects CODEOWNERS under repo path."""
        codeowners = tmp_path / "CODEOWNERS"
        codeowners.write_text("/my-chart/ @org/backend\n")

        controller = ChartsController(repo_path=tmp_path)
        assert controller.team_mapper is not None
        assert controller._extract_team({}, chart_name="my-chart") == "Backend"
...
class TestChartsControllerAnalysis:
    ...
    def test_analyze_limit_request_ratios_empty(self, controller: ChartsController) -> None:
        """Test analyze_limit_request_ratios with empty list."""
        charts = []
        result = controller.analyze_limit_request_ratios(charts)
        assert result == []

    def test_analyze_limit_request_ratios_below_threshold(self, controller: ChartsController) -> None:
        """Test analyze_limit_request_ratios when ratios are below threshold."""
        from kubeagle.models.charts.chart_info import ChartInfo
        ...
        charts = [
            ChartInfo(
                ...
                cpu_request=100,
                cpu_limit=150,  # ratio = 1.5
                memory_request=128,
                memory_limit=192,  # ratio = 1.5
        ...
        result = controller.analyze_limit_request_ratios(charts, threshold=2.0)
        assert result == []

    def test_analyze_limit_request_ratios_above_threshold(self, controller: ChartsController) -> None:
        """Test analyze_limit_request_ratios when ratios exceed threshold."""
        from kubeagle.models.charts.chart_info import ChartInfo
        ...
        charts = [
            ChartInfo(
                ...
                values_file="values.yaml",
                cpu_request=100,
                cpu_limit=500,  # ratio = 5.0
                memory_request=128,
                memory_limit=512,  # ratio = 4.0
                qos_class=QoSClass.BURSTABLE,
                has_liveness=True,
                has_readiness=True,
                has_startup=False,
        ...
        result = controller.analyze_limit_request_ratios(charts, threshold=2.0)
        assert len(result) == 1
        assert result[0].chart_name == "chart1"
        assert result[0].cpu_ratio == 5.0
        assert result[0].memory_ratio == 4.0
        assert result[0].max_ratio == 5.0

    def test_get_team_statistics_empty(self, controller: ChartsController) -> None:
        """Test get_team_statistics with empty list."""
        result = controller.get_team_statistics([])
        assert result == []

    def test_get_team_statistics_single_chart(self, controller: ChartsController) -> None:
        """Test get_team_statistics with single chart."""
        from kubeagle.models.charts.chart_info import ChartInfo
        ...
        result = controller.get_team_statistics(charts)
        assert len(result) == 1
        assert result[0].team_name == "team-a"
        assert result[0].chart_count == 1
        assert result[0].cpu_request == 100
        assert result[0].cpu_limit == 200

    def test_detect_charts_without_pdb_empty(self, controller: ChartsController) -> None:
        """Test detect_charts_without_pdb with empty list."""
        result = controller.detect_charts_without_pdb([])
        assert result == []

    def test_detect_charts_without_pdb_all_have_pdb(self, controller: ChartsController) -> None:
        """Test detect_charts_without_pdb when all charts have PDB."""
        from kubeagle.models.charts.chart_info import ChartInfo
        ...
        result = controller.detect_charts_without_pdb(charts)
        assert result == []

    def test_detect_charts_without_pdb_finds_missing(self, controller: ChartsController) -> None:
        """Test detect_charts_without_pdb finds charts without PDB."""
        from kubeagle.models.charts.chart_info import ChartInfo
        ...
        result = controller.detect_charts_without_pdb(charts)
        assert len(result) == 1
        assert result[0]["name"] == "chart-no-pdb"
        assert result[0]["team"] == "team-a"

    def test_get_charts_without_pdb_by_team(self, controller: ChartsController) -> None:
        """Test get_charts_without_pdb_by_team groups by team."""
        from kubeagle.models.charts.chart_info import ChartInfo
        ...
        result = controller.get_charts_without_pdb_by_team(charts)
        assert "team-a" in result
        assert "team-b" in result
        assert len(result["team-a"]) == 1
        assert len(result["team-b"]) == 1

    def test_analyze_all_charts_filters_main_by_parent_name(
        self, tmp_path: Path
    ) -> None:
        """Parent release filter should still include `<service>/main` charts."""
        main_chart = tmp_path / "contact-service" / "main"
        main_chart.mkdir(parents=True)
        (main_chart / "Chart.yaml").write_text("name: main")
        (main_chart / "values.yaml").write_text("{}")

        other_chart = tmp_path / "other-service"
        other_chart.mkdir()
        (other_chart / "Chart.yaml").write_text("name: other-service")
        (other_chart / "values.yaml").write_text("{}")

        controller = ChartsController(repo_path=tmp_path, max_workers=2)
        result = controller.analyze_all_charts(active_releases={"contact-service"})

        assert len(result) == 1
        assert result[0].name == "main"

    def test_analyze_all_charts_filters_using_chart_yaml_name(
        self, tmp_path: Path
    ) -> None:
        """Active release filtering should honor Chart.yaml metadata name."""
        metadata_chart = tmp_path / "folder-name"
        metadata_chart.mkdir(parents=True)
        (metadata_chart / "Chart.yaml").write_text("name: release-name\n")
        (metadata_chart / "values.yaml").write_text("{}")
        ...
        controller = ChartsController(repo_path=tmp_path, max_workers=2)
        result = controller.analyze_all_charts(active_releases={"release-name"})

        assert len(result) == 1
        assert result[0].name == "release-name"

    def test_analyze_all_charts_includes_all_values_file_variants(
        self, tmp_path: Path
    ) -> None:
        """Each values*.yaml file should produce its own analyzed chart row."""
        chart_path = tmp_path / "ab-template-preview"
        chart_path.mkdir(parents=True)
        (chart_path / "Chart.yaml").write_text("name: ab-template-preview")
        (chart_path / "values.yaml").write_text("pdb:\n  enabled: false\n")
        (chart_path / "values-automation.yaml").write_text(
            "pdb:\n  enabled: false\n"
        )
        (chart_path / "values-default-namespace.yaml").write_text(
            "pdb:\n  enabled: false\n"
        )

        controller = ChartsController(repo_path=tmp_path, max_workers=2)
        result = controller.analyze_all_charts()

        assert len(result) == 3
        assert [Path(chart.values_file).name for chart in result] == [
