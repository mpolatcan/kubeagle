...
class AIFullFixBulkModal(ModalScreen[AIFullFixBulkModalResult | None]):
    ...
    def _render_status_chips(self, bundle: ChartBundleEditorState) -> None:
        chip_items = self._status_chip_items(bundle)
        with contextlib.suppress(Exception):
            container = self.query_one("#ai-full-fix-bulk-status-chips", CustomContainer)
            container.remove_children()
            for key, value in chip_items:
                chip_title = escape(str(key or "Status").strip() or "Status")
                chip_text = self._chip_display_value(str(key or ""), str(value or ""))
                chip_value = _highlight_status_keywords(chip_text or "N/A")
                chip_widget = CustomStatic(
                    f"[bold]{chip_title}[/bold]\n{chip_value}",
                    classes="ai-full-fix-status-chip",
                    markup=True,
                )
                tooltip_text = self._chip_tooltip(str(key or ""), str(value or ""))
                if tooltip_text:
                    chip_widget.tooltip = tooltip_text
                container.mount(chip_widget)

    def _status_chip_items(self, bundle: ChartBundleEditorState) -> list[tuple[str, str]]:
        provider_key = self._resolved_provider_key(bundle.status_text)
        provider = self._display_provider_name(provider_key)
        model_label, model_value = self._active_model_chip(
            status_text=bundle.status_text,
            provider_key=provider_key,
        )
        completion_time = self._completion_time_label(bundle)
        items: list[tuple[str, str]] = [
            ("AI Fix Provider", provider),
            (model_label, model_value),
            ("Status", self._fix_status_label(bundle)),
            ("Total Completion Time", completion_time),
        ]
        render_verification = self._status_value(bundle.status_text, "Render Verification")
        verification_details = self._status_value(bundle.status_text, "Verification Details")
        if render_verification:
            items.append(("Render Verification", render_verification))
        if verification_details:
            items.append(("Verification Details", verification_details))
        for key, value in self._status_entries(bundle.status_text):
    ...
    def _resolved_provider_key(self, status_text: str) -> str:
        status_provider = self._status_value(status_text, "Provider").strip().lower()
        if status_provider in {"claude", "codex"}:
            return status_provider
        settings = getattr(self.app, "settings", None)
        settings_provider = str(getattr(settings, "ai_fix_llm_provider", "codex") or "codex").strip().lower()
        if settings_provider in {"claude", "codex"}:
            return settings_provider
        return "auto"

    def _active_model_chip(self, *, status_text: str, provider_key: str) -> tuple[str, str]:
        status_model = self._status_value(status_text, "Model")
        if status_model.strip():
            model_name = self._display_model_name(status_model)
        else:
            codex_model, claude_model = self._settings_model_chip_values()
    ...
    def _settings_model_chip_values(self) -> tuple[str, str]:
        settings = getattr(self.app, "settings", None)
        codex_raw = str(getattr(settings, "ai_fix_codex_model", "auto") or "auto")
        claude_raw = str(getattr(settings, "ai_fix_claude_model", "auto") or "auto")
        return (
            self._display_model_name(codex_raw),
            self._display_model_name(claude_raw),
        )

    @staticmethod
    def _status_entries(status_text: str) -> list[tuple[str, str]]:
        ...
        for raw_line in str(status_text or "").splitlines():
            ...
            if not line:
                continue
            key, has_separator, value = line.partition(":")
            if has_separator and key.strip():
                entries.append((key.strip(), value.strip()))
                continue
            entries.append(("Status", line))
        return entries
